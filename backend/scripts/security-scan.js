...
const allowedFiles = new Set(['.env.example', 'SECURITY_NOTES.md', 'security-scan.js', 'smoke-check.js']);

const patterns = [
  ['client_gemini_global', /window\.GEMINI_API_KEY/],
  ['google_api_key_literal', /AIza[0-9A-Za-z_-]{20,}/],
  ['openai_key_literal', /sk-[0-9A-Za-z_-]{20,}/],
  ['non_example_gemini_env_assignment', /GEMINI_API_KEY\s*=\s*(?!<)[^\s#]+/],
  ['non_example_amadeus_secret_assignment', /AMADEUS_API_SECRET\s*=\s*(?!<)[^\s#]+/],
